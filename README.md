...
8. [Cấu hình `.env.local`](#cấu-hình-envlocal)
...
Tạo file `docker-compose.yml` tại `/opt/ql_tro/` (hoặc import vào Portainer → **Stacks → Add stack**):
...
Xem nội dung đầy đủ ở mục [Cấu hình .env.local](#cấu-hình-envlocal) bên dưới.
...
# Production:   postgresql://postgres:postgres@localhost:5432/ql_tro
POSTGRESQL_URI=postgresql://postgres:postgres@localhost:5432/ql_tro
...
`NEXTAUTH_URL` trong `.env.local` phải là URL **public** (Zalo sẽ gọi vào `NEXTAUTH_URL/api/zalo/webhook`).
...
```bash
grep POSTGRESQL_URI /opt/ql_tro/.env.local
# Nếu thiếu:
...
### File `.env.local`
- [ ] `POSTGRESQL_URI` kết nối được database
