...
import { ArrowLeft, Info, Image, Users, RefreshCw, Loader2 } from 'lucide-react';
...
export default function ChinhSuaPhongPage() {
  const router = useRouter();
  const params = useParams();
  const phongId = params.id as string;
  ...
  // Khách thuê hiện tại
  const [currentKhachThueName, setCurrentKhachThueName] = useState('');
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [phongRes, toaNhaRes, khachThueRes] = await Promise.all([
      ...
      if (phongRes.ok) {
        ...
        if (phongData.success) {
          const phong = phongData.data.find((p: any) => p.id === phongId);
          if (phong) {
            const getToaNhaId = (toaNha: string | { id: string }) => {
              if (typeof toaNha === 'object' && toaNha !== null) return toaNha.id || '';
            ...
            if (currentHopDong?.khachThue?.length > 0) {
              setCurrentKhachThueName(currentHopDong.khachThue.map((k: any) => k.hoTen).join(', '));
            }
          } else {
      ...
      if (toaNhaRes.ok) {
        ...
        if (toaNhaData.success) setToaNhaList(toaNhaData.data);
      }

      if (khachThueRes.ok) {
        const ktData = await khachThueRes.json();
        if (ktData.success) {
          setAvailableKhachThue(ktData.data
  ...
  async function handleAssignKhachThue() {
    if (!selectedKhachThueId) return;
    setAssigningKhachThue(true);
    try {
      const res = await fetch(`/api/phong/${phongId}/thue`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleUnassignKhachThue() {
    setAssigningKhachThue(true);
    try {
      const res = await fetch(`/api/phong/${phongId}/thue`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
        toast.success('Đã hủy gán khách thuê');
        setCurrentKhachThueName('');
        fetchData();
      } else {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div>
          <h1 className="text-lg md:text-xl font-semibold">Chỉnh sửa phòng: {formData.maPhong}</h1>
          <p className="text-xs md:text-sm text-gray-500">Cập nhật thông tin phòng</p>
        </div>
      </div>
