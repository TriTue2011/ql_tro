...
const schema = z
  .object({
    ...
    thumbnailUrl: z.string().min(1, "URL thumbnail không hợp lệ").optional(), // thumbnail cho video (tùy chọn)
...
async function getAppBaseUrls(): Promise<{
  ...
  try {
    ...
    const map = Object.fromEntries(
      rows.map((row) => [row.khoa, row.giaTri?.trim() || ""]),
    );
    return {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (await isBotServerMode()) {
      ...
      const fixedImageUrl = await normalizeAssetUrl(
        imageUrl,
        request,
        "bot_server",
      );
      const fixedFileUrl = await normalizeAssetUrl(
        fileUrl,
        request,
        "bot_server",
      );
      const fixedVideoUrl = await normalizeAssetUrl(
        videoUrl,
        request,
        "bot_server",
      );
      const fixedThumbnailUrl = await normalizeAssetUrl(
        thumbnailUrl,
        request,
        "bot_server",
      );
    ...
    if (normalizedImageUrl) {
      const response = await sendZaloPhoto(
        token,
        chatId,
        normalizedImageUrl,
        message,
