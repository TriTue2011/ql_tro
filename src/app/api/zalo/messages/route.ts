...
export async function GET(request: NextRequest) {
  ...
  // ownId = ID tài khoản Zalo của user. Fallback sang userId để tương thích webhook cũ.
  const userOwnId = nguoiDung?.zaloAccountId || userId;
  ...
  if (searchParams.get("conversations") === "1") {
    const rows = await prisma.$queryRaw<any[]>`
      WITH latest_user AS (
        ...
        FROM "ZaloMessage"
        WHERE "role" = 'user' AND "ownId" = ${userOwnId}
      ...
      latest_bot AS (
        ...
        FROM "ZaloMessage"
        WHERE "role" = 'bot' AND "ownId" = ${userOwnId}
        ORDER BY COALESCE("rawPayload"->>'threadId', "chatId"), "createdAt" DESC
      )
  ...
  // Kiểm tra quyền truy cập — cuộc hội thoại phải thuộc ownId của user
